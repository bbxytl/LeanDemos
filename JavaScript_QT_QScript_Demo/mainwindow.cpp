...
void MainWindow::Test()
{
    ...
    m_objects->clear();
    m_objects->setCon(con);
#ifndef M_OBJECTS
#define M_OBJECTS
    //向JS里添加全局变量
    QScriptValue object= m_engine.newQObject(m_objects);   //创建一个全局对象
    global.setProperty("object",object);


//    QScriptValue mesClass = m_engine.scriptValueFromQMetaObject<JSDataSet>();

//    global.setProperty("js",mesClass);
#endif
    //调用执行JavaScript里的函数
...
{
    m_js=m_objects->getDSObject(customDataId);
...
//    QString templateFile="E:/Documents/My-Doc/YinLei--WorkDoc/myReports/\
            工程量清单项目预算表--ok-14121509-1.xml";
#if 0
